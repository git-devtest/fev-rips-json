...
const camposConsulta = [
  body('cod_consulta')
    .trim().notEmpty().withMessage('cod_consulta es requerido')
    .isLength({ max: 10 }),
  ...
  body('cod_dx_principal')
    .trim().notEmpty().withMessage('cod_dx_principal es requerido')
    .isLength({ max: 10 }),
  ...
  body('finalidad_consulta')
    .trim().notEmpty().withMessage('finalidad_consulta es requerida')
    .isIn(['01','02','03','04','05','06','07','08','09'])
    .withMessage('Finalidad de consulta no válida'),

  body('causa_externa')
    .trim().notEmpty().withMessage('causa_externa es requerida')
  ...
  body('tipo_doc_id_medico').optional({ nullable: true })
  ...
  body('cod_dx_relacionado1').optional({ nullable: true }).isLength({ max: 10 }),
  body('cod_dx_relacionado2').optional({ nullable: true }).isLength({ max: 10 }),
  body('cod_dx_relacionado3').optional({ nullable: true }).isLength({ max: 10 }),
  body('cod_cup_causa_externa').optional({ nullable: true }).isLength({ max: 10 }),

  // Valores monetarios
  body('valor_consulta').optional().isFloat({ min: 0 }).toFloat(),
...
const crearConsulta    = [facturaIdParam, usuarioIdParam, ...camposConsulta];
const actualizarConsulta = [facturaIdParam, usuarioIdParam, idParam,
  ...camposConsulta.map(r => r.optional ? r : r) // todos opcionales en PATCH
];

// ─── Rutas ────────────────────────────────────────────────────────────────────
// Montadas bajo /api/v1/facturas/:factura_id/usuarios/:usuario_id/consultas

router.get('/',      [facturaIdParam, usuarioIdParam], ctrl.index);
router.get('/:id',   [facturaIdParam, usuarioIdParam, idParam], ctrl.show);
router.post('/',     crearConsulta,     ctrl.create);
router.patch('/:id', actualizarConsulta, ctrl.update);
